...
export const testSuitePrice = () => describe("MarketPlace price", () => {
  let createEditionTransaction,  
    ...
    test("idPrice returns nil, when get price of non-existent sale", async () => {
        let error;
        try {
          const second = await getAccountAddress("second");
    ...
    test("idPrice returns price", async () => {
        let error;
        try {
            const admin = await getAccountAddress("admin");
            const second = await getAccountAddress("second");

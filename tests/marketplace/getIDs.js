...
export const testSuiteIDs = () => describe("MarketPlace get IDs", () => {
  let createEditionTransaction,  
    ...
    test("getIDs return empty array, when sales do not exist on account", async () => {
        let error;
        try {
          const second = await getAccountAddress("second");
    ...
    test("getIDs return array of sales", async () => {
        let error;
        try {
            const admin = await getAccountAddress("admin");
            const second = await getAccountAddress("second");
            ...
            //Get IDs script 
            const sales = await executeScript({
                code: getIDsScript,
                args: [
                    // Owner of NFT
                    [second, t.Address],          
            ...
            // Check sales in blockchain
            expect(sales).toEqual([NFTId]);       
        } catch (e) {     
           error = e;
        }
        expect(error).toEqual(undefined);

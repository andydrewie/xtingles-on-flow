...
export const testSuiteChangePrice = () => describe("MarketPlace change price", () => {
  let createEditionTransaction,  
    ...
    test("changePrice throws error, when change price to non-existent sale", async () => {
        let error;
        try {
            const second = await getAccountAddress("second");
            const NFTId = 1;
    ...
    test("changePrice throws error, when new price is 0", async () => {
        let error;
        try {
            const admin = await getAccountAddress("admin");
            const second = await getAccountAddress("second");

...
export const testSuiteCommissionPaymentsOpenEdition = () => describe("Open Edition commission", () => {
    ...
    test("commission payments check events during failed pay commission", async () => { 
        ...
        try {
            ...
            const earnedEvents = events.filter(event => event.type === `A.${admin.substr(2)}.OpenEditionV3.Earned`);     

            const failEarnedEvents = events.filter(event => event.type === `A.${admin.substr(2)}.OpenEditionV3.FailEarned`);    

...
export const testSuiteSendBidTokens = () => describe("Bid auction", () => {
  ...
  beforeEach(async (done) => {
    ...
    test("return FUSD tokens to the previous bidder in case cancel auction", async () => { 
      try {
          ...
          expect(parseInt(bidEvents[0].data.amount, 10)).toEqual(50);

          // 2. TokensDeposited to the admin account
          expect(bidEvents[1].type).toEqual(`A.${admin.substr(2)}.FUSD.TokensDeposited`);
          expect(bidEvents[1].data.to).toEqual(admin);
          expect(parseInt(bidEvents[1].data.amount, 10)).toEqual(50);


          // The cancel
          // 2. TokensWithdrawn from the admin account. Return the previous bid
          expect(cancelEvents[1].type).toEqual(`A.${admin.substr(2)}.FUSD.TokensWithdrawn`);
          expect(cancelEvents[1].data.from).toEqual(admin);
          expect(parseInt(cancelEvents[1].data.amount, 10)).toEqual(50);

          // 3. TokensDeposited to the second account. Return the previous bid
          expect(cancelEvents[2].type).toEqual(`A.${admin.substr(2)}.FUSD.TokensDeposited`);
          expect(cancelEvents[2].data.to).toEqual(second);
          expect(parseInt(cancelEvents[2].data.amount, 10)).toEqual(50);

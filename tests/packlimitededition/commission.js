...
export const testSuiteCommissionPaymentsLimitedEdition = () => describe("Limited Edition commission", () => {
    ...
    test("commission payments check events during failed pay commission", async () => { 
        ...
        try {
            ...
            const earnedEvents = events.filter(event => event.type === `A.${admin.substr(2)}.PackLimitedEdition.Earned`);     

            const failEarnedEvents = events.filter(event => event.type === `A.${admin.substr(2)}.PackLimitedEdition.FailEarned`);    

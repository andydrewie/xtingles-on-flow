...
pub contract Auction {
    ...
    pub resource AuctionItem {
        ...
        //the time the acution should stASMR at
        ...
        pub fun placeBid(bidTokens: @FungibleToken.Vault, vaultCap: Capability<&{FungibleToken.Receiver}>, collectionCap: Capability<&{ASMR.CollectionPublic}>) {
            ...
            if bidderAddress != collectionAddress {
              panic("you cannot make a bid and send the ASMR to sombody elses collection")
            }
    ...
    pub resource AuctionCollection: AuctionPublic {
        ...
        pub fun settleAuction(_ id: UInt64, cutPercentage: UFix64) {
            let itemRef = &self.auctionItems[id] as &AuctionItem
            itemRef.settleAuction(cutPercentage: cutPercentage, cutVault: self.marketplaceVault)
        }
    ...
    //this method is used to create a standalone auction that is not pASMR of a collection
    //we use this to create the unique pASMR of the Versus contract
